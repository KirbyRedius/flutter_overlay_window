## 0.5.3
- Захуярил WindowManager.addView в try/catch. addView может вылететь на реальных устройствах с DeadObjectException (system_server транзакция упала), отозванным разрешением SYSTEM_ALERT_WINDOW, OEM-ограничениями на оверлеи или когда дисплей пропал. Раньше сервис просто ронял весь процесс при такой хуйне; теперь логируем ошибку, подчищаем за собой оверлей и останавливаем сервис.

## 0.5.2 
- Удалил свой захардкоженный код (когда-то добавил для хотфикса, охуеть)
